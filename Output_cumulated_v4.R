...
for (s in c("summer","winter","year")) {
  for (t in c("IA")) {
    ...
    ggsave(paste0(outdir,"/cumulated_",t,"_deaths_agegroups_",s,"_v4", ifelse(IArest, "_IArestricted", ""), ".png"), width = 20, height = 30, units = "cm")
    if (population) {
      ggplot(data = out, aes(x = wk, y = cm, colour = season)) + geom_line()  +
        ...
        theme(axis.text.x = element_text(size = 10, angle = 0),
              ...
              plot.caption = element_text(hjust = 0)) +
        facet_wrap( ~ agegrp.labels, nrow = 5, scales = "free_y")
      ggsave(paste0(outdir,"/cumulated_",t,"_mr_agegroups_",s,"_v4", ifelse(IArest, "_IArestricted", ""), ".png"), width = 20, height = 30, units = "cm")
    }
    for (a in 0:4) {
      ggplot(data = out[(out$agegrp==a),], aes(x = wk, y = cd, colour = season)) + geom_line()  +
        ...
        theme(axis.text.x = element_text(size = 10, angle = 0),
              ...
              plot.title = element_text(hjust = 0.5, vjust=2),
              plot.caption = element_text(hjust = 0))
      ggsave(paste0(outdir,"/cumulated_",t,"_deaths_agegroup_",a,"_",s,"_v4", ifelse(IArest, "_IArestricted", ""), ".png"), width = 30, height = 20, units = "cm")
      if (population) {
        ggplot(data = out[(out$agegrp==a),], aes(x = wk, y = cm, colour = season)) + geom_line()  +
          ...
          theme(axis.text.x = element_text(size = 10, angle = 0),
                ...
                plot.title = element_text(hjust = 0.5, vjust=2),
                plot.caption = element_text(hjust = 0))
        ggsave(paste0(outdir,"/cumulated_",t,"_mr_agegroup_",a,"_",s,"_v4", ifelse(IArest, "_IArestricted", ""), ".png"), width = 30, height = 20, units = "cm")
      }
    }
  }
}
